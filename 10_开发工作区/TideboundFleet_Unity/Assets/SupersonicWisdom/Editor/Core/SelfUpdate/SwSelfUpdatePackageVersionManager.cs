...
namespace SupersonicWisdomSDK.Editor
{
    internal static class SwSelfUpdatePackageVersionManager
    {
        ...
        internal static async Task<(int, long)> FetchVersionDetailsForTitle(bool isInitiatedByUser)
        {
            ...
            var responseDictionary = SwUtils.JsonHandler.DeserializeObject<Dictionary<string,object>>(responseString);
        ...
        internal static async Task<SwSelfUpdateRawConfiguration> FetchUpdateConfig()
        {
            ...
            {
                SwEditorTracker.TrackEditorEvent(nameof(FetchUpdateConfig), ESwEditorWisdomLogType.SelfUpdate, ESwEventSeverity.Error, "Error fetching self update config.");
                SwEditorLogger.LogError(e);
                return SwUtils.JsonHandler.DeserializeObject<SwSelfUpdateRawConfiguration>(SwEditorConstants.EMPTY_STRINGIFIED_JSON);
            }
