...
namespace SupersonicWisdomSDK.Editor
{
    internal class SwTrackEventScheduler
    {
        ...
        private void LoadEventsFromDisk()
        {
            ...
            try
            {
                _eventQueue = SwUtils.JsonHandler.DeserializeObject<ConcurrentQueue<Dictionary<string, object>>>(decryptedContent);
                SwEditorLogger.Log($"{nameof(SwTrackEventScheduler)} | {nameof(LoadEventsFromDisk)} | Loaded events from disk");
            } 
            catch (Exception e)
            {
                SwEditorLogger.LogError($"{nameof(SwTrackEventScheduler)} | {nameof(LoadEventsFromDisk)} | Failed to load events from disk with exception: {e}");
            }
        }

        private async Task SerializeQueueToDisk()
        {
            if(_eventQueue.SwIsNullOrEmpty()) return;
            
            var serializedQueue = EncryptFile(SwUtils.JsonHandler.SerializeObject(_eventQueue));
