...
## 当前修订：奖励目标结算与分享（V2-A4）

用户要求移除结算的“第一海域3/10”等海域统计，改为直观展示可获取奖励的进度，并增加分享；同时去掉主页已通关进度条。主页保留开始／继续按钮里的当前关号，不再显示已通关比例。此条覆盖下面历史结构中的海域统计与章节条要求。

结算显示已保存的首通／战斗金币、展示船、通关外观奖励节点，以及主页／下一关／分享。奖励模块围绕“还差几关获得什么”呈现：最近目标的图标、名称、已通关量／目标关卡、未达成／新解锁／已解锁状态；不能冒充排行榜或增加新货币。节点名称和5／8／10关仅为当前预览的演示配置，不写入生产奖励表。正式版本从解锁配置与可靠通关凭证计算，并与图鉴保持同源。

分享为可选次操作；当前只打开分享卡预览，系统分享由后续Unity平台适配实现。没有确认分享金币奖励，不照搬参考图“分享+200”；打开／取消／失败不能影响结算、解锁或下一关。分享卡默认只含游戏名、关号与本作展示图，不包括账号或私人数据。保存失败时先重试，不展示成功分享或已解锁结果。见[奖励结算修订](REWARD_RESULT_REVISION.md)。
...
沿用真实3D船体＋正俯视正交相机＋低成本接触阴影＋2D HUD。V1已跑通，不改局内相机俯角、不重写Grid输入、不重新做收藏服务。先把信息分层和页面结构做好，再用一小组视觉样片定风格。推荐参考稿中的明快玩具舰队方向，具体水色、按钮、字体与船体材质仍是候选。
...
主界面参考已核查录屏：中央大角色换成本作旗舰、主要按钮在底部、角色可低幅活动，按钮和文字保持稳定；不复制竞品角色与品牌。结算保留下一关主操作与可获取奖励的进度，取消海域完成统计。第2关领取蓝皮先回主界面，再显式装备／开始第3关；第10关按普通关卡结算：有下一关则继续，暂未开放则留在结算提示，不自动回主页，详见[流程修订](FLOW_REFINEMENT.md)。每日奖励、邀请有礼、排行榜按A3在主页预留，业务实现后置；不加入假排名或付费促销。
...
| 奖励追赶感 | 展示最近可获取外观及通关进度，不展示海域完成率或虚构排名 |
...
|---|---|---|---|---|
| P00 主界面 | 可更换展示船（无游戏标题）；已结算金币与券；主按钮里的继续位置，无已通关条 | 开始／继续挑战 | 左右两侧图鉴／抽奖／补给与后续功能；设置在顶部 | 新增轻量Home视图，状态从原存档投影；不是另建钱包或关卡进度 |
...
| P03 结算 | 通关；首通／战斗／合计；已入账；外观奖励节点进度 | 下一关；第2关待领时返回主界面领取蓝皮 | 返回主界面、分享预览 | VictoryResult／匹配SettlementRecord；不按现价重算 |
| P04 奖励概览 | 已解锁／下一奖励，未配置时明确说明 | 返回主页 | 查看奖励详情 | 由通关解锁表与可靠进度投影；替代预览稿旧航程统计 |
...
首页顶部只常驻钱包／券和设置；旗舰居中；不显示已通关比例／进度条；底部单一主按钮；左侧图鉴／抽奖／补给，右侧预留每日奖励／邀请／排行。抽奖不常驻红点闪烁，不让收藏页打断连闯。线稿不演示摇摆，正式样片再比较幅度与减少动态版本。
...
| 完全死局 | 全部不可移动且队列结束后局内短提示与可用道具指引；不弹窗，不自动消耗或重开 | 无弹跳 |
