...
主界面参考已核查录屏：中央大角色换成本作旗舰、主要按钮在底部、角色可低幅活动，按钮和文字保持稳定；不复制竞品角色与品牌。结算保留一个主操作和一条真实章节进度。第2关领取蓝皮先回主界面，再显式装备／开始第3关；第10关显示当前内容完成。签到、活动、假排名、付费促销仍不加入这一批。

## 2. 参考到实施的边界

| 参考建议 | 本轮定稿约束 |
|---|---|
| 船有体积、背景安静 | 复用V1模型与共享资源；线稿中的箭头只说明布局，不是最终船图 |
...
|---|---|---|---|---|
| P00 主界面 | 游戏名／旗舰；已结算金币与券；当前海域；继续位置 | 开始／继续挑战 | 抽奖、图鉴／装备、道具补给；设置收角落 | 新增轻量Home视图，状态从原存档投影；不是另建钱包或关卡进度 |
...
| P05 图鉴 | 收藏数（注明含默认）、5槽、卡片品质／拥有／装备状态 | 选一艘查看 | 品质／来源筛选；返回主界面 | 从主界面进入，CollectionPanel拆视图仍调用原收藏服务；16款目录不增减 |
...
首页顶部只常驻钱包／券和设置；旗舰居中；海域进度为次级；底部单一主按钮，下面三入口“抽奖／图鉴·装备／道具补给”。抽奖不常驻红点闪烁，不让收藏页打断连闯。线稿不演示摇摆，正式样片再比较幅度与减少动态版本。
...
3. **V2-D 结算／收藏／商店接入**：按P03～P09替换视图，补短屏滚动、十连十项、满槽替换、异常恢复与后台回归；不激活C3经济参数。
