...
| 第10关结果 | 示例三项均已解锁；保留下一关／主页／分享。有后续则继续；未开放则留在结算提示，见[最新流程修订](FLOW_REFINEMENT.md) |
...
额外核对3→5目标差额、解锁详情标记、第5关解锁与图鉴一致、分享预览／关闭／Escape不改钱包、下一关推进、第10关无第11关（此历史交互已被最新FLOW_REFINEMENT修订覆盖）、保存失败重试后开放分享，以及主页无已通关条。目视核对三档关键结果、分享卡和深色结果。本轮未运行Unity测试，浏览器单位不是设备触控验收。
