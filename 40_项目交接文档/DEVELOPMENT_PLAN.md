...
版本：2.1；日期：2026-09-18。每个阶段只有在入口条件满足后启动，不在前一阶段夹带下一阶段功能。Phase 5 以后以高密度方向疏通体验为主线，详细计划见 [PHASE5_PLUS_PLAN.md](PHASE5_PLUS_PLAN.md)。
...
| Phase 5 单船迁移与 80 船灰盒 | 未开始 | 合并原 Phase 4.5，并验证正式玩法密度 | 单基础船、12×18、80 船可清盘灰盒、100 船压力夹具、可读性与性能门槛 |
...
- 当前 8×9 校验上限只是实现事实，Phase 5 目标上限为 12×18。
- 正式关总船数不超过 82、长船不超过 8、占用格不超过 172，并至少保留 44 个空格。
- 使用默认灰盒船验证 80 艘，不等待最终船皮肤、Boss 美术、UI 或广告。
- 第 1 关保持约 7 艘教学；第 2 关直接使用约 80 艘正式数量。
- Phase 5 未通过方向可读性、触控和最低设备性能前，不启动 Phase 6–7。
...
- 当前 Phase 1–4 提交仍是旧结构和小规模测试的实现事实。此记录不授权直接修改代码；Phase 5 开发前必须先产出迁移清单、80 船灰盒方案和测试更新范围。
